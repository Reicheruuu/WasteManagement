...
# Load YOLOv5 models
model1 = torch.hub.load('ultralytics/yolov5', 'custom', path='main model/weights/best.pt')
...
while True:
    ...
    for i in range(len(filtered_detections1)):
        ...
        if confidence >= confidence_threshold:
            # Determine the category and send the corresponding byte command to Arduino
            if category == 'Biodegradable':
                ser.write(b'1')  # Byte command for biodegradable
            elif category == 'Non-Biodegradable':
                ser.write(b'2')  # Byte command for non-biodegradable
            elif category == 'Recyclable':
                ser.write(b'3')  # Byte command for recyclable

            color = (0, 255, 0) if category == 'Biodegradable' else (
            255, 255, 0) if category == 'Non-Biodegradable' else (255, 0, 0)
